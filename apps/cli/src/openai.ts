/**
 * OpenAI GPT credential management for the `dsh` CLI.
 *
 * The optional OpenAI GPT provider (`openai-codex` in pi-ai) authenticates
 * through either ChatGPT OAuth (browser or device code) or an OpenAI Platform
 * API key. The credential document lives at `$DSH_HOME/pi-ai-auth.json`, the
 * same owner-only file the `llm-pi-ai` provider route reads for requests, so
 * a login here is immediately visible to a running or later Harness session.
...
async function chooseLoginMethod(requested: string | undefined): Promise<OpenAiLoginMethod> {
  ...
  try {
    ...
    const methods: Record<string, OpenAiLoginMethod> = { '1': 'browser', '2': 'device', '3': 'api-key' }
    const method = methods[answer]
...
async function loginOAuth(method: 'browser' | 'device'): Promise<void> {
...
export async function loginOpenAi(method: string | undefined): Promise<void> {
  ...
  if (resolved === 'api-key') return loginApiKey()
  return loginOAuth(resolved)
}
...
/** Emit the OpenAI GPT login state as one JSON object (for programmatic use). */
export async function openAiStatusJson(): Promise<void> {
  const credentials = new PiAiCredentialStore(dshHomePath('pi-ai-auth.json'))
  const credential = await credentials.read(PROVIDER_ID)
  if (credential?.type === 'oauth') {
    const models = createModels({ credentials })
