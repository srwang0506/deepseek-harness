...
裸 `dsh` 打开交互式终端客户端：一个全屏 Ink 客户端，流式打印会话事件（assistant 文本、reasoning、工具调用与带 diff 着色的结果），以封闭的 y/n 选项行内回答审批，以预设编号、键入的自定义答案或多行自由文本行内回答提问，并处理 `/new`、`/resume [id]`、`/sessions`、`/model [model]`、`/status`、`/permissions [preset]`、`/compact`、`/init`、`/doctor`、`/export`、`/diff`、`/undo`、`/help`、`/quit`、`/exit` 这些斜杠命令。`dsh --resume <id>`（或 `dsh --continue`）续跑持久化会话，并在提示符前重放其转录。`/status` 报告完整会话状态（身份、模型与推理强度、带工作区根的 sandbox 模式、审批策略、权限预设与 token 用量），`/permissions [preset]` 显示或切换会话的权限预设；底部状态栏常驻显示模型、sandbox 模式与预设。第一次 Ctrl+C 只取消当前轮次；Ctrl+D（与 `/quit`、`/exit` 一样）退出并 flush；轮次运行中提交的一行会在下一步边界引导 Agent。`dsh resume` 打开会话选择器浏览持久化会话（标题从会话日志折叠而来，方向键移动，Enter 续跑，`f` 分叉选中项，Esc 取消）；`dsh resume --last` 续跑最近的会话，`dsh resume <id>` 续跑指定会话。带任务位置参数时，`dsh exec "任务"`（裸 `dsh "任务"` 为别名）以一次性模式运行同一 runner：打印最终文本并退出。裸 `dsh exec` 从管道 stdin 读取任务，在终端 stdin 上以用法诊断失败。`--jsonl` 输出版本化流（`init` 信封、每个已提交事件一条带 `v` 的行、收尾 `result` 行）；`--output-schema`（内联 JSON 或文件路径）用 harness 支持的 JSON Schema 子集校验最终输出——不匹配时向 stderr 打印违规项并以 2 退出；`-o <path>` 把最终输出写入文件而非 stdout。退出码：0 完成，1 其他结局，2 schema 不匹配。没有任务且 stdin 非终端时，以 1 退出并给出用法诊断。
...
可选的 OpenAI GPT 提供方通过 `dsh login` 登录（浏览器 OAuth、设备码 OAuth 或 OpenAI Platform API key），通过 `dsh status`（仅登录状态——会话完整状态在 TUI 的 `/status` 中）或 `dsh login status` 查看状态，通过 `dsh logout` 退出。凭据保存在 `$DSH_HOME/pi-ai-auth.json`，与 `llm-pi-ai` 提供方路由读取的同一个 owner-only 文档一致。`dsh model` 显示或选择默认 agent 模型：`dsh model`（或 `model status`）打印当前选择，`dsh model deepseek` 选择 DeepSeek V4 Flash，`dsh model gpt [model] [effort]` 选择 OpenAI GPT 路由，`dsh model <provider>/<model>` 选择任意已配置路由。stdin 为终端时，不带方式调用 `dsh login` 会提示选择；否则必须显式指定方式。
