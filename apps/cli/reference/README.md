...
The optional OpenAI GPT provider authenticates through `dsh login` (browser OAuth, device-code OAuth, or an OpenAI Platform API key), reports its state through `dsh status`, and clears it through `dsh logout`. The credential lives at `$DSH_HOME/pi-ai-auth.json`, the same owner-only document the `llm-pi-ai` provider route reads for requests. `dsh model` shows or selects the default agent model: `dsh model` (or `model status`) prints the current selection, `dsh model deepseek` selects DeepSeek V4 Flash, `dsh model gpt [model] [effort]` selects an OpenAI GPT route, and `dsh model <provider>/<model>` selects any configured route. `dsh login` with no method prompts for a choice when stdin is a terminal, and requires an explicit method otherwise.
