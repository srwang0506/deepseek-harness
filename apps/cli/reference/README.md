...
Bare `dsh` opens the interactive terminal client: a full-screen Ink client that streams session events (assistant text, reasoning, tool calls, and diff-colored results), answers approvals inline as a closed y/n choice and questions as preset numbers, typed custom answers, or multi-line free text, and handles the slash commands `/new`, `/resume [id]`, `/sessions`, `/model [model]`, `/status`, `/permissions [preset]`, `/compact`, `/init`, `/doctor`, `/export`, `/diff`, `/undo`, `/help`, `/quit`, and `/exit`. `dsh --resume <id>` (or `dsh --continue`) resumes a persisted session and replays its transcript before the prompt. `/status` reports the complete session state (identity, model and reasoning effort, sandbox mode with its workspace root, approval policy, permission preset, and token usage) and `/permissions [preset]` shows or switches the session's permission preset; the bottom status bar always shows the model, sandbox mode, and preset. The first Ctrl+C cancels only the running turn; Ctrl+D (like `/quit` and `/exit`) quits and flushes; a line submitted while a turn runs steers the agent at its next step boundary. `dsh resume` opens a session picker over persisted sessions (titles folded from the session log, arrow keys to move, Enter to resume, `f` to fork the selection, Esc to cancel); `dsh resume --last` resumes the most recent session, and `dsh resume <id>` resumes the named one. With a task positional, `dsh exec "task"` (bare `dsh "task"` is an alias) runs the same runner in one-shot mode: print the final text and exit. A bare `dsh exec` reads the task from piped stdin and fails with a usage diagnostic on a terminal stdin. `--jsonl` emits a versioned stream (`init` envelope, one `v`-stamped line per committed event, closing `result` line), `--output-schema` (inline JSON or a file path) validates the final output against the harness's supported JSON Schema subset — a mismatch prints the violations on stderr and exits 2 — and `-o <path>` writes the final output to a file instead of stdout. Exit codes: 0 completed, 1 any other ending, 2 schema mismatch. With no task and a non-terminal stdin, it exits 1 with a usage diagnostic.
...
## OpenAI GPT login, model selection, and third-party providers

The optional OpenAI GPT provider authenticates through `dsh login` (browser OAuth, device-code OAuth, or an OpenAI Platform API key), reports its state through `dsh status` (login state only — the session's full state lives in the TUI's `/status`) or `dsh login status`, and clears it through `dsh logout`. The credential lives at `$DSH_HOME/pi-ai-auth.json`, the same owner-only document the `llm-pi-ai` provider route reads for requests. `dsh model` shows or selects the default agent model: `dsh model` (or `model status`) prints the current selection, `dsh model deepseek` selects DeepSeek V4 Flash, `dsh model gpt [model] [effort]` selects an OpenAI GPT route, and `dsh model <provider>/<model>` selects any configured route. `dsh login` with no method prompts for a choice when stdin is a terminal, and requires an explicit method otherwise.
