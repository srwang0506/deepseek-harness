...
Configure credentials, the model catalog, and deployment-specific transport settings per provider, keyed by the provider route itself. `apiKeyEnv` is a credential *reference* resolved per request, so no secret enters this file. Omitting it leaves authentication to the installed pi-ai provider: key-capable routes may use their native ambient discovery, while OAuth routes read an explicitly configured `credentialStorePath`. An embedding application may also store either OAuth or API-key credentials for the logical `openai-codex` route. A configured key reference that resolves to nothing fails with `MISSING_CREDENTIAL` instead of falling through to an unrelated ambient key. One credential serves every model on its route.
...
The top-level OAuth field is `credentialStorePath`. Supported profile fields are `apiKeyEnv`, `displayName`, `api`, `baseURL`, `models`, `modelOverrides`, `compat`, `defaultContextWindow`, `defaultMaxTokens`, `defaultInput`, `headers`, `reasoning`, `thinkingBudgets`, `cacheRetention`, `openAIResponses`, `transport`, `timeoutMs`, `websocketConnectTimeoutMs`, `streamIdleTimeoutMs`, and `retryPolicy`. Each profile's optional retry policy is captured with that provider route; omission uses bounded normal defaults. The stream-idle interval is a positive finite Node timer delay, defaults to five minutes, and covers only an outstanding provider read, not consumer think time. Harness app attribution wins a conflicting configured header name.

`openAIResponses` is a first-party Responses-only control block. `store` writes response state at OpenAI; `previousResponseId` requires `store: true` and, when the latest durable assistant response belongs to the exact same route and model, sends its response id as `previous_response_id` and replaces that provider-owned prefix with only the later Harness messages. Any foreign or incompatible assistant response falls back to full durable history. `reasoningContext: all_turns` sends `reasoning.context: "all_turns"`. These controls do not change Chat Completions or the ChatGPT Codex OAuth transport. A stored API key on the logical `openai-codex` route selects the standard OpenAI Responses transport, where the controls do apply.
...
API keys never enter the provider definitions. The harness resolves a route's key through its own seam before the request reaches pi-ai and passes it as the request's `apiKey` option, which pi-ai treats as the highest-priority auth override. When `credentialStorePath` is configured, the snapshot also holds the persistent store for provider-native credentials. OAuth uses it for locked refresh. If the logical `openai-codex` route instead stores an API key, the adapter rebinds the selected catalog model to pi-ai's standard `openai` Responses provider for that request, passes the key as the request override, and records replay metadata under the original route and model. A route naming no key and storing no credential leaves authentication to its provider.
...
- **Stored login UI is composition-owned** — the adapter can consume stored OAuth or API-key credentials and refresh provider-native OAuth credentials, but it cannot invent the interaction surface that obtains them. The generic configurable-provider directory therefore withholds OAuth-only catalog providers until a composition explicitly declares the route and supplies `credentialStorePath`; that composition must call pi-ai `Models.login()` or write a validated canonical API-key credential itself. `openai-codex` is the installed catalog's OAuth-only transport route.
