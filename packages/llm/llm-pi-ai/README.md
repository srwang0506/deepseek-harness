...
- id: llm
  ...
  config:
    ...
    providers:
      # OAuth catalog route. Login orchestration is owned by the embedding app;
      # Models uses this store for request auth and locked token refresh.
      openai-codex:
...
API keys never enter that collection. The harness resolves a route's key through its own seam before the request reaches pi-ai and passes it as the request's `apiKey` option, which pi-ai treats as the highest-priority auth override. When `credentialStorePath` is configured, the collection does hold the persistent store used solely for provider-native stored credentials such as OAuth; this gives pi-ai locked refresh without changing the Harness key-reference semantics. A route naming no key leaves authentication to its provider.
...
- **OAuth login UI is composition-owned** — the adapter can persist and refresh provider-native OAuth credentials, but it cannot invent the interaction surface that obtains them. The generic configurable-provider directory therefore withholds OAuth-only catalog providers until a composition explicitly declares the route and supplies `credentialStorePath`; that composition must call pi-ai `Models.login()` itself. `openai-codex` is the installed catalog's OAuth-only route.
