 ...
 * method — the authentication a generic settings card can configure today.
 *
 * A key is what the harness resolves through its own credential seam and hands
 * pi-ai per request. OAuth instead requires an explicitly configured persistent
 * credential store plus an app-owned login surface. The generic provider
 * directory cannot promise those composition facts, so it still withholds an
 * OAuth-only catalog route until a profile explicitly declares that route.
 * @param provider - provider route key.
