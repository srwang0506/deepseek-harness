...
普通 `tui-startup` 提供方（[`src/startup.ts`](src/startup.ts)）注入 `ctx.cmdlineArgs`（[`dsh-cmdline`](../../boot/cmdline/README.md)），解析本应用的命令行：一次性任务位置参数、`--resume <id>`、`--continue`、`--model <model>`、`--json`、`--jsonl` 与 `--help`。runner 注入该服务，再从惰性配置中读取本次调用。
