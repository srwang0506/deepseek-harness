...
- **composer 仍是行编辑器，而非 Codex 的 composer 编辑器**：回到上一条用户消息修改并分叉，留待后续；当前编辑面为 ↑↓ 历史调出、`@path` Tab 补全、`@` 模糊文件搜索、Vim 动作子集与 Tab 排队下一轮。
