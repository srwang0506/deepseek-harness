...
runner（[`src/index.ts`](src/index.ts)）有两种模式，共享同一个 `TerminalSessionController`（[`src/controller.ts`](src/controller.ts)）：它持有 Agent 的采用与销毁、实时事件流以及审批/提问应答器——轮次运行中提交的输入会在下一步边界处引导（steer）Agent，轮次运行中按 Tab 把该行排入下一轮（状态栏显示 `⇥ queued` 标记直到队列排空），空 composer 上按 ↑ 打开消息编辑浮层——选择一条历史用户消息、修改后提交，即从该消息所在轮次分叉会话（分叉经由会话持久化后端落盘，子会话与任何持久化会话一样续跑），`shutdown()` 则取消正在运行的轮次、flush、销毁并解除监听。当 `task` 非空时，它通过 `ctx.agents` 驱动一个全新或续跑的 Agent，等待停稳、flush Session、打印最后一条非空 assistant 文本，最终轮次完成则退出码 0，否则 1。没有任务时，它挂载一个全屏 Ink 应用（[`src/ui/app.tsx`](src/ui/app.tsx)），运行在可观察 store（[`src/ui/store.ts`](src/ui/store.ts)）之上：把会话事件流式打印到终端（assistant 文本、reasoning 与带 diff 着色的结果；用户行以 `› `（bold dim）、assistant/reasoning/工具行以 `• `（dim）呈现，Codex 式，工具名标注每个调用行、其结果行即卡片正文，轮次间以 dim `─` 分隔，工具轮次标注 `Local tools: N calls`），以封闭的 y/n 选项行内应答审批（Esc、Ctrl+D 或轮次取消会把该请求按 cancelled 驳回），以预设编号、键入的自定义答案或多行自由文本行内回答提问（多选同时支持勾选预设与附加文本），用 Tab 补全把 `@path` 提及解析为文件内容，行首输入 `@` 时打开项目文件模糊搜索（键入过滤、↑/↓ 选择、Enter 把 `@path` 提及插入到项目文件索引之上），通过带光标的编辑引擎与 Vim normal 模式子集编辑 composer 行（多行粘贴原样插入并作为一条消息提交；合并的尾部 Enter 仍提交单行输入）（Esc；`h`/`l`、`0`/`$`、`w`/`b`、`x`、`D`、`i`/`a`/`I`/`A`）并显示可见块光标，用 ↑↓ 调出提交过的输入历史，用 `Ctrl+R` 搜索提交过的提示词历史（模糊过滤浮层：键入过滤、↑/↓ 选择、Enter 复用该行），用 `!` 前缀执行本地 shell 命令，从 `$DSH_HOME/commands/*.md` 加载用户定义的提示词模板命令，用提示中的 `$name` token 调用技能（技能的渲染指令以 `skill-invocation` 消息注入，与模型自行调用 `skill` 工具的路径一致），并处理 `/new`、`/fork`、`/delete [id]`、`/resume [id]`、`/sessions`、`/model [model]`、`/reasoning [effort]`（off/high/max；`default` 清除）、`/login [method]`、`/logout`、`/status`、`/permissions [preset]`、`/compact`、`/init`、`/doctor`、`/export`、`/diff`、`/review`、`/undo`、`/help`、`/quit`、`/exit` 这些斜杠命令；不带参数的 `/resume`（以及启动器的 `dsh resume`）会打开会话选择器，列出带折叠标题的持久化会话——方向键移动高亮，Enter 续跑，`f` 分叉选中项（持久化会话经由 agents 注册表加载、分叉并销毁源会话），Esc 取消——而 `dsh resume --last` 续跑最近的会话；`/status` 报告完整会话状态——身份、模型与推理强度、带工作区根的 sandbox 模式、审批策略、权限预设与实时 token 用量——`/permissions [preset]` 则显示或切换会话的权限预设（持久化会话事件，续跑时恢复）；第一次 Ctrl+C 只取消当前轮次，而 Ctrl+D（与 `/quit`、`/exit` 一样）退出并 flush。模型选择是会话级的：续跑会话会从其折叠的 `request/context` 事件恢复路由、从最后一条请求头恢复推理强度，底部 footer（普通 composer 行下方的暗色 Codex 式一行）左侧显示 `? for shortcuts`（或运行中的 braille spinner），右侧为分段着色状态行：模型（青）、实时 token 用量（绿）、git 分支（品红）、生效中的 sandbox 模式（品红），`-m` 启动覆盖会加标；终端窗口/标签标题以 Codex 风格设置（`dsh | <标题或模型> | <分支>`），会话标题生成后更新、退出时清除；新会话打开时先显示 Codex 式带边框标题卡（`>_ dsh (vX)`、带推理强度的模型、相对化为 `~` 的目录，内宽钳制 56 列并对长路径居中截断），随后是 Codex onboarding 块（`To get started…`，然后是各以两空格缩进、` - ` 分隔的 `/init`、`/status`、`/permissions`、`/model`、`/review`）；父链到达当前根会话的 subagent 会话会以带标签的后台行渲染（工具调用、最终消息、失败）。；`/login` 会挂起 Ink 界面，在原始终端上运行浏览器/设备码/API key 三种 OpenAI GPT 登录流程。续跑的会话会在提示符前以带样式的 Markdown 重放其持久化转录。
...
- **composer 仍是行编辑器，而非 Codex 的 composer 编辑器**：当前编辑面为 ↑↓ 历史调出、`@path` Tab 补全、`@` 模糊文件搜索、Vim 动作子集、Tab 排队下一轮与编辑历史消息并分叉。
