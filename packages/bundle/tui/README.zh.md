...
- **composer 仍是行编辑器，而非 Codex 的 composer 编辑器**：模糊文件搜索、`Ctrl+R` 历史搜索、用 Tab 排队下一轮、回到上一条用户消息修改并分叉，均留待后续；当前编辑面为 ↑↓ 历史调出、`@path` Tab 补全与 Vim 动作子集。
