...
- **The composer is a line editor, not Codex's composer editor** — editing a prior user message to fork from it is deferred; ↑↓ history recall, `@path` Tab completion, the `@` fuzzy file search, the Vim motion subset, and Tab queueing of the next turn are the current editing surface.
