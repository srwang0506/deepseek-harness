...
- **The composer is a line editor, not Codex's composer editor** — fuzzy file search, `Ctrl+R` history search, queueing the next turn with Tab, and editing a prior user message to fork from it are deferred; ↑↓ history recall, `@path` Tab completion, and the Vim motion subset are the current editing surface.
