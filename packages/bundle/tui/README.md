...
- **The composer is a line editor, not Codex's composer editor** — fuzzy file search, `Ctrl+R` history search, Vim mode, queueing the next turn with Tab, and editing a prior user message to fork from it are deferred; ↑↓ history recall and `@path` Tab completion are the current editing surface.
