...
async function bench(script: Script): Promise<Bench> {
  ...
  return {
    ...
    run: async () => {
      ...
      apply(ctx, { task: 'do the thing', resumeSessionId: '', continue: false, model: '' })
...
describe('tui runner (one-shot)', () => {
  ...
  it('validates config defaults', () => {
    expect(new Config({ task: 'x', resumeSessionId: '', continue: false, model: '' })).toMatchObject({ task: 'x', resumeSessionId: '', continue: false, model: '' })
  })
})
