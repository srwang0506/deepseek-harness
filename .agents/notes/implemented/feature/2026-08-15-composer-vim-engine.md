...
`composer.ts` is unit-covered at 100% (17 tests: insertion at cursor, backspace/Delete, clamping, word motions, mode toggles, submission, coalesced Enter chunks, control keys). The PTY e2e gained a `raw` driver op for keys with no visible echo and a scenario that types `abcd`, Esc, `h`, `x`, Esc, `e` and asserts `abce` reaches the model. The help text and READMEs document the key map; the Known Limitations list drops Vim mode from the deferred composer work.
