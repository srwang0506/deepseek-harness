...
The repository ships an Apple-silicon desktop build path whose native AppKit launcher embeds Node, the built Harness web application, a production dependency closure, the official Codex CLI, and an OpenAI-first Cordis patch. It starts a private loopback web server on an operating-system-assigned port, keeps the physical process cwd on APFS while assigning `/Volumes/sirui/deepseek-harness` as the preferred logical workspace across the host, filesystem, and sandbox services, keeps deployment state and logs in the user's Library, terminates its child server when the application quits, and is packaged as the ad-hoc-signed `deepseek harness.app` plus a metadata-free ZIP suitable for storage on exFAT.
