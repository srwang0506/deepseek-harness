...
A fresh installation starts and remains usable with DeepSeek before any OpenAI login. Selecting GPT automatically presents the three authentication methods; after one succeeds, GPT remains a normal provider/model choice and every Harness-native capability stays in the same loop. The desktop owns a secret-bearing file, browser and device-code login interaction, and API-key entry, so file permissions, standard-input key transfer, atomic writes, refresh locking, logout, and attributable login errors are part of its security boundary. ChatGPT and API-key requests intentionally use different transports and billing relationships even though the model selector keeps one logical route.
