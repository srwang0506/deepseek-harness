...
`composer.ts` 单元覆盖率 100%（17 个用例：光标处插入、backspace/Delete、边界钳制、单词动作、模式切换、提交、合并 Enter chunk、控制键）。PTY e2e 为无可见回显的按键新增了 `raw` 驱动操作，并新增场景：输入 `abcd`、Esc、`h`、`x`、Esc、`e`，断言 `abce` 到达模型。帮助文本与 README 记录键位；Known Limitations 将 Vim 模式移出暂缓的 composer 事项。
